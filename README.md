...
В рамках курса я изучал и применял на практике различные аспекты работы с базами данных. Это серия из 12 самостоятельных и групповых работ, каждая из которых посвящена определенной теме. Я решал реальные задачи, связанные с созданием схем баз данных, написанием запросов разной сложности, оптимизацией производительности и работой с данными в многопользовательской среде.
...
```bash
git clone https://github.com/ldvsked/school21-sql.git
```

2. Перейдите в папку нужного проекта:
```bash
cd school21-sql/SQLB1_Basics
```
